                                             ...
                                             Error* error)
{
  ...
  switch (target_language)
  {
    ...
    case GPUShaderLanguage::GLSLES:
    {
      ...
      if (ubos_count > 0)
      {
        // Set name of the UBO to match our shaders, so that drivers without binding info can still find it.
        dyn_libs::spvc_compiler_set_name(scompiler, ubos[0].id, "UBOBlock");
      }

      if (push_constants_count > 0)
      {
        // Set name of the push constant block to match our shaders, so that drivers without binding info can still find it.
