...
inline bool PostProcessing::SlangShaderPreprocessor::ParseFile(std::string_view base_path, std::string_view path)
{
  ...
  SlangShaderStage::Id previous_stage = m_current_stage;
  ...
  m_current_stage = SlangShaderStage::Common;
  ...
  m_current_stage = previous_stage;
                                                  ...
                                                  u32 idx, bool is_final_pass, Error* error)
{
  ...
  // TODO: frame_count_mod, texture mipmap
                                                      ...
                                                      bool push_constant, Error* error)
{
  ...
  if (push_constant)
  {
    DEV_LOG("Pass '{}' has push constants '{}' of size {} bytes", pass.name, resource.name, struct_size);
    pass.push_constants_size = static_cast<u32>(struct_size);
  ...
  {
    DEV_LOG("Pass '{}' has uniform buffer '{}' of size {} bytes", pass.name, resource.name, struct_size);
    pass.uniforms_size = static_cast<u32>(struct_size);
  }
                                                            ...
                                                            float time)
{
  ...
  for (size_t i = 0; i < m_passes.size(); i++)
  {
    ...
    BindPassUniforms(pass, input_color, output_size, final_rect, orig_width, orig_height, native_width, native_height,
                     target_width, target_height, time);
    ...
    if (output_texture.generate_mipmaps)
    {
                                                ...
                                                u32 viewport_height, Error* error)
{
  ...
  for (Pass& pass : m_passes)
  {
    ...
    if (!tex.texture || tex.texture->GetWidth() != tex_width || tex.texture->GetHeight() != tex_height)
    {
      ...
      const GPUTexture::Flags flags =
        tex.generate_mipmaps ? GPUTexture::Flags::AllowGenerateMipmaps : GPUTexture::Flags::None;
      g_gpu_device->RecycleTexture(std::move(tex.texture));
      g_gpu_device->RecycleTexture(std::move(tex.feedback_texture));
...
void PostProcessing::SlangShader::BindPassUniforms(const Pass& pass, GPUTexture* input_color, GSVector2i output_size,
                                                   ...
                                                   u32 target_height, float time)
{
  ...
  for (const Uniform& ui : pass.uniforms)
  {
    ...
    if (ui.type < 0)
    {
      switch (ui.type)
      {
        ...
        {
          std::memcpy(dst, &m_frame_count, sizeof(u32));
        }
        break;
  ...
  if (pass.push_constants_size > 0)
    g_gpu_device->PushUniformBuffer(push_constant_data, pass.push_constants_size);
}
